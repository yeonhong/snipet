...
namespace com.t7t.formation
{
	...
	public class FormationGrid : MonoBehaviour
	{
		...
		[SerializeField]
		protected Transform anchorTransform => anchor.transform;
		[SerializeField]
		protected Vector3 anchorPosition => anchor.transform.position;
		[SerializeField]
		protected Quaternion anchorRotation => anchor.transform.rotation;
		#endregion
				...
				if ((oldPosition - anchorPosition).sqrMagnitude > 0.001f * 0.001f)  // TODO: potentially we can do this by checking if target has been reached
{
				...
				if (positionDirty) {
					...
					if (randomizeOffset > 0.0F) {                                       // Randomize the positions slightly if enabled
